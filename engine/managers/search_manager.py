...
def find_named_network_object(devices: list, name: str = "", addr: str = ""):
    ...
    if len(name) != 0:
        for x in devices:
            if x.type == DeviceTypes.ASA:
                objs.append(AsaNetworkObject(x.ip_addr, obj_name=name))

    elif len(addr) != 0:
        for x in devices:
            if x.type == DeviceTypes.ASA:
                objs.append(AsaNetworkObject(x.ip_addr, ip_addr=ip_address(addr)))
    else:
        return None
